...
{ m_##__FIELD__ = l;}
        ...
        void bind_event_one(std::string const& event_name,event_listener const& func)
        {
    ...
    void client::bind_event_one(std::string const& event_name,event_listener const& func)
    {
        m_impl->bind_event_one(event_name,func);
    }
    
    void client::bind_event(std::string const& event_name,event_listener_aux const& func)
    {
        m_impl->bind_event_one(event_name,event_adapter::do_adapt(func));
    }
    ...
    void client::impl::clear_timers()
    {
        ...
        dlog_print(DLOG_ERROR, LOG_TAG, "clear timers");
        boost::system::error_code ec;
        if(m_ping_timeout_timer)
        {
